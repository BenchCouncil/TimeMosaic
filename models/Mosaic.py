...
class Model(nn.Module):
    def __init__(self, configs, patch_len=16, stride=8):
        ...
        self.patch_num = int(configs.seq_len // self.patch_len_list[0])
    ...
    def forecast(self, x_enc, segment):
        ...
        if self.revin:
            ...
            dec_out = dec_out * stdev[:, 0, :].unsqueeze(1).repeat(1, int(self.num_segs * self.seg_len), 1)
            dec_out = dec_out + means[:, 0, :].unsqueeze(1).repeat(1, int(self.num_segs * self.seg_len), 1)

        return dec_out, self.patch_embedding.latest_cls_soft 

    def forward(self, x_enc, segment=0):
        dec_out, cls_soft = self.forecast(x_enc, segment)
        if self.training > 0:
            return dec_out[:, -self.pred_len:, :], cls_soft
        else:
