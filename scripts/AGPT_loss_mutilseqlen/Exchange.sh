...
batch_size_map[336]=32
batch_size_map[720]=128

for seq_len in "${seq_lens[@]}"; do
  for pred_len in "${pred_lens[@]}"; do

    # 仅当 pred_len ≤ seq_len 时运行
    if [ "$pred_len" -le "$seq_len" ]; then
      e_layers=${e_layers_map[$pred_len]}
      n_heads=${n_heads_map[$pred_len]}
      batch_size=${batch_size_map[$pred_len]}

      echo "Running ETTm2 with seq_len=$seq_len, pred_len=$pred_len, e_layers=$e_layers, n_heads=$n_heads, batch_size=$batch_size"
